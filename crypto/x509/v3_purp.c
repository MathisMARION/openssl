...
static X509_PURPOSE xstandard[] = {
    ...
    {X509_PURPOSE_CODE_SIGN, X509_TRUST_OBJECT_SIGN, 0,
     ...
     NULL},
    {X509_PURPOSE_WISUN_FAN, X509_PURPOSE_WISUN_FAN, 0,
                                   ...
                                   int non_leaf)
{
    ...
    /*
     * b. KeyUsage - MUST have digitalSignature bit set, MAY have keyAgreement
     *    bit set
     */
    if ((x->ex_kusage & KU_DIGITAL_SIGNATURE) == 0)
        return 0;
     ...
     *    ii.  MUST contain the object identifier id-kp-clientAuth (defined in
     *         [RFC3280]).
     *    iii. MAY contain other key usages, but those are not specified by
     *         this standard.
     */
    if ((x->ex_flags & EXFLAG_XKUSAGE) == 0)
        return 0;
